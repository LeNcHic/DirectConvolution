...
template <typename T>
bool img_equal(const T *img1, int stride1, const T *img2, int stride2, int height, int width, int channels, T eps = T()) {
  const T *ptr1 = img1;
  const T *ptr2 = img2;
...
template <>
bool img_equal<real32_t>(const real32_t *img1, int stride1, const real32_t *img2, int stride2, int height, int width, int channels, real32_t eps) {
    ...
    switch (ct){
      ...
      case CONV_TYPE_TILING_LOOP1:
        loop_order_n1_tiled(result.data(), input_image.data(), matrix.data(),
                      in_size.height, in_size.width, in_channels, stride.height, stride.width, out_channels,
                      filter_size.height, filter_size.width, 16);
        break;
...
void ConvWrapper::init() {
  ...
  std::generate(matrix.data(), matrix.data() + out_channels * in_channels / groups * filter_size.height * filter_size.width, generator);
}
...
MUSTINLINE void report(const ConvWrapper& cw, double time, double error, double total_time, int n_runs){
  ...
  << std::setw(10)<< error << " ms (" << error / time * 100
            << "%) -- totally " << total_time << " s in " << n_runs << " runs" << std::endl;
}
  ...
  for(int c = 0; c < cycles; ++c) {
    for (int kernel_size: {3, 5}) {
      for (int size: {50, 70, 85, 100}) {
        for (int in_channels: {8, 16, 32, 64, 128}) {
          for (int out_channels: {8, 16, 32, 64, 128}) {
            std::cout << "TESTING convolution of " << size << "x" << size << "x" << in_channels << " image with "
                      << kernel_size << "x" << kernel_size << "x" << in_channels << "x" << out_channels << " kernel" << std::endl;
            ConvWrapper a;
